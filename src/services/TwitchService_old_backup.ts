...
export class TwitchService {
  ...
  public async validateToken(): Promise<boolean> {
    try {
      logger.debug('Twitchトークンの有効性をチェック中...', '🔑');
        logger.debug('Twitch APIリクエスト: GET /users');
      ...
      logger.debug(`Twitch APIレスポンス: ${response.status} /users`);      if (response.status === 200) {
  ...
  public async getStreamStatus(channelName: string): Promise<TwitchStream | null> {
    try {
      ...
      logger.debug('Twitch APIリクエスト: GET /streams');
      logger.debug(`Twitch APIレスポンス: ${response.status} /streams`);
  ...
  private async getPlaylistUrlMethod1(channelName: string): Promise<string | null> {
    try {
      ...
      const response = await axios.post('https://gql.twitch.tv/gql', queryPayload, {
        headers: {
          'Client-ID': clientId,
      ...
      if (!tokenData?.value || !tokenData?.signature) {
        logger.debug(`方法1: トークンデータが無効または取得失敗。レスポンス: ${JSON.stringify(response.data)}`);
        return null;
      }
      ...
      let errorMessage = `方法1失敗: ${error.message}`;
      if (error.response) {
        errorMessage += ` Status: ${error.response.status}, Data: ${JSON.stringify(error.response.data)}`;
      } else if (error.request) {
        errorMessage += ' リクエストは行われましたが、レスポンスがありませんでした。';
      }
  ...
  private async getPlaylistUrlMethod2(channelName: string): Promise<string | null> {
    try {
      ...
      const tokenData = response.data?.data?.streamPlaybackAccessToken;
      if (!tokenData?.value || !tokenData?.signature) {
        logger.debug(`方法2: トークンデータが無効または取得失敗。レスポンス: ${JSON.stringify(response.data)}`);
        return null;
      }
      ...
      let errorMessage = `方法2失敗: ${error.message}`;
      if (error.response) {
        errorMessage += ` Status: ${error.response.status}, Data: ${JSON.stringify(error.response.data)}`;
      } else if (error.request) {
        errorMessage += ' リクエストは行われましたが、レスポンスがありませんでした。';
  ...
  private async getPlaylistUrlFallback(channelName: string): Promise<string | null> {
    try {
      ...
      if (testResponse.status === 200) {
        ...
        logger.debug(`方法3: HEADリクエスト成功せず。Status: ${testResponse.status}`);
        return null;
      }

    } catch (error: any) {
      let errorMessage = `方法3失敗: ${error.message}`;
      if (error.response) {
        errorMessage += ` Status: ${error.response.status}, Data: ${JSON.stringify(error.response.data)}`;
      } else if (error.request) {
        errorMessage += ' リクエストは行われましたが、レスポンスがありませんでした。';
